...
export function setupAuth(app: Express) {
  ...
  // This app's database is Replit's built-in Postgres, which runs without SSL
  // (sslmode=disable). Only enable SSL for external managed providers.
  const dbIsLocal =
    /@(localhost|127\.0\.0\.1|helium)[:/]/.test(connectionString || "") ||
    /sslmode=disable/.test(connectionString || "");
  const pool = new pg.Pool({
    connectionString,
    ...(dbIsLocal ? {} : { ssl: { rejectUnauthorized: false } }),
  });
  ...
  app.use(
    session({
      store: pgStore,
      secret: process.env.SESSION_SECRET || "basic-evolutionary-psychology-secret-key",
      resave: false,
  ...
  if (googleEnabled) {
    ...
    // server through the shared path-based proxy (only /api/* reaches it).
    // Register this URI in the owner's Google Cloud Console OAuth client.
    ...
    const getCallbackURL = () => {
      if (process.env.NODE_ENV === "production") {
        const prodDomain = (process.env.REPLIT_DOMAINS || "")
          .split(",")[0]
          ?.trim();
        return `https://${prodDomain || "21b9d182-41dc-4157-838f-867891770258-00-1ba6553y8nvjz.kirk.replit.dev"}${CALLBACK_PATH}`;
      }
    ...
    // Legacy alias in case the non-api-prefixed URI is registered instead
